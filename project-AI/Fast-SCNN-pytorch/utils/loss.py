...
class SoftmaxCrossEntropyOHEMLoss(nn.Module):
    ...
    def forward(self, predict, target, weight=None):
        ...
        with torch.no_grad():
            ...
            if 0 < self.min_kept < num_valid:
                prob = torch.softmax(predict.detach(), dim=1)
                safe_target = target.clamp_min(0).unsqueeze(1)
                target_prob = prob.gather(1, safe_target).squeeze(1)[valid_mask]

                threshold = torch.as_tensor(self.thresh, device=predict.device, dtype=target_prob.dtype)
                kth = torch.kthvalue(target_prob, min(self.min_kept, target_prob.numel())).values
                threshold = torch.maximum(threshold, kth)

                kept_mask = valid_mask.clone()
                kept_mask[valid_mask] = target_prob <= threshold
                target = target.clone()
                target[~kept_mask] = self.ignore_label
